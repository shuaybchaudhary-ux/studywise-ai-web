...
import { AuthForm, type AuthFormValues } from '../login/components/auth-form';
...
export default function SignupPage() {
  ...
  const handleSubmit = async (values: AuthFormValues) => {
    const success = await createUserWithEmailAndPassword(values.email, values.password);
    if (success) {
      router.push('/chat');
    }
  };
  ...
  return (
    <div className="flex min-h-screen flex-col justify-center items-center bg-background px-4">
      <div className="w-full max-w-md">
        ...
        <p className="text-center text-muted-foreground mb-6">Start your AI-powered learning journey today.</p>
